...
ElfFile64::ElfFile64(unsigned char* pszPathName) {
...
void ElfFile64::initall() {
	...
	try
	{
		pEhdr = (Elf64_Ehdr*)baseAddr;
		m_dwShNum = pEhdr->e_shnum;
		pSectionHdr = (Elf64_Shdr*)(baseAddr + pEhdr->e_shoff);
		m_dwPhNum = pEhdr->e_phnum;
		pProgmHdr = (Elf64_Phdr*)(baseAddr + pEhdr->e_phoff);
	}
	catch (...)
	{
	}
...
//魔法数（0x7f “ELF”）
BOOL  ElfFile64::getElfMagic(char szBuf[MAXBYTE]) {
...
//Elf Class 32 64
BOOL  ElfFile64::getElfClass(char szBuf[MAXBYTE]) {
...
//Elf 编码方式
BOOL  ElfFile64::getElfEncode(char szBuf[MAXBYTE]) {
...
/* number of versions */
//获取Elf 的版本
BOOL  ElfFile64::getElfVersion(char szBuf[MAXBYTE]) {
...
//获取对齐值，标记未使用字节的开始
BOOL  ElfFile64::getElfABI(char szBuf[MAXBYTE]) {
...
void ElfFile64::printRemain() {
	printf("虚拟入口地址：   e_entry: %I64d\r\n"
		"程序表偏移：     e_phoff: %I64d\r\n"
		"节表偏移：       e_shoff: %I64d\r\n"
...
void ElfFile64::printPhdType(int p_Type) {
	char* psz = "unused";
	switch (p_Type) {
...
void ElfFile64::printShdType(int p_Type) {
	char* psz = " inactive ";
	switch (p_Type) {
...
int ElfFile64::getTargetFuncInfo(const char *funcName, funcInfo32 *info) {
	char flag = -1, *dynstr;
	int i;
	Elf64_Off dyn_vaddr;
	Elf64_Word dyn_size, dyn_strsz;
	Elf64_Dyn *dyn;
	Elf64_Addr dyn_symtab, dyn_strtab, dyn_hash;
	Elf64_Sym *funSym;
	unsigned funHash, nbucket;
	unsigned *bucket, *chain;
	int mod;
	Elf64_Phdr* phdr = pProgmHdr;
	...
	dyn_vaddr = (Elf64_Addr)(nOffset + baseAddr);
	dyn_size = phdr->p_filesz;
	flag = 0;
	for (dyn = (Elf64_Dyn*)dyn_vaddr; (Elf64_Addr)dyn < dyn_vaddr + dyn_size; dyn++) {
	...
	dyn_hash = Rva2Fa(dyn_hash) + (Elf64_Addr)baseAddr;
	dyn_strtab = Rva2Fa(dyn_strtab) + (Elf64_Addr)baseAddr;
	dyn_symtab = Rva2Fa(dyn_symtab) + (Elf64_Addr)baseAddr;
// 	dyn_hash += (Elf64_Addr)baseAddr;
// 	dyn_strtab += (Elf64_Addr)baseAddr;
// 	dyn_symtab += (Elf64_Addr)baseAddr;
	funHash = elf_hash(funcName);
	funSym = (Elf64_Sym *)dyn_symtab;
	...
	funcInfo64* info2 = (funcInfo64*)info;
	info->st_value = funSym[i].st_value;
	info->st_size = funSym[i].st_size;
	return 0;
}

void ElfFile64::printFuncInfo(const char *funcName) {
	funcInfo64 info;
	getTargetFuncInfo(funcName, (funcInfo32*)&info);
	printf("offset:%0x, size:%0x", info.st_value, info.st_size);
}

Elf64_Addr ElfFile64::Rva2Fa(Elf64_Addr rva) {
	Elf64_Phdr* phdr = pProgmHdr;
